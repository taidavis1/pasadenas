const ServicesData = [
    {
        id : 1,
        'servicename': 'Manicure',
        'service': [
            {
                'name': 'Manicure-Classic (15 mins)',
                'price': '$25',
                "imgUrl": require('../../img/services/manicure/1.jpg')
                
            },
            {
                'name': 'Manicure Gel And Spa (30mins)',
                'price': '$55',
                "imgUrl": require('../../img/services/manicure/2.jpg')
                
            },
            {
                'name': 'Change The Basic (10 mins)',
                'price': '$15',
                "imgUrl": require('../../img/services/manicure/3.jpg')
            },
            {
                'name': 'Manicure-Gel (20 mins)',
                'price': '$40',
                "imgUrl": require('../../img/services/manicure/4.jpg')
            },
            {
                'name': 'Manicure Buffer Shiny',
                'price': '$35',
                "imgUrl": require('../../img/services/manicure/5.jpg')
            },
        ],
        'imgPath': require('../../img/services/manicure/1.jpg')
    },
    {
        id : 2,
        'servicename': 'Pedicure',
        'service': [
            {
                'name': 'Pedicure Basic (30 Mins)',
                'price': '$37',
                "imgUrl": require('../../img/services/pedicure/1.jpg')
            },
            {
                'name': 'Pedicure-College (45 Mins)',
                'price': '$55',
                "imgUrl": require('../../img/services/pedicure/2.jpg')
            },
            {
                'name': 'Pedicure-Honey (55 Mins)',
                'price': '$70',
                "imgUrl": require('../../img/services/pedicure/3.jpg')
            },
            {
                'name': 'Pedicure-VIP (70 Mins)',
                'price': '$90',
                "imgUrl": require('../../img/services/pedicure/4.jpg')
            },
            {
                'name': 'Pedicure Deluxe  (35 Mins)',
                'price': '$50',
                "imgUrl": require('../../img/services/pedicure/5.jpg')
            },
            {
                'name': 'Pedicure-Hot Stone (50 Mins)',
                'price': '$60',
                "imgUrl": require('../../img/services/pedicure/6.jpg')
            },
            {
                'name': 'Pedicure-LuXury Spa (55 Mins)',
                'price': '$80',
                "imgUrl": require('../../img/services/pedicure/7.jpg')
            },
            {
                'name': 'Relax Spa(35 Mins)',
                'price': '$45',
                "imgUrl": require('../../img/services/pedicure/5.jpg')
            },
        ],
        "imgPath": require('../../img/services/pedicure/1.jpg')
    },
    {
        id : 3,
        'servicename': 'Additional Extra',
        'service': [
            {
                'name': 'Herbal Neck Treatment',
                'price': '$15',
                "imgUrl": require('../../img/services/polishAdd/1.jpg')
            },
            {
                'name': 'Gel Polish Application',
                'price': '$20',
                "imgUrl": require('../../img/services/polishAdd/2.jpg')
            },
            {
                'name': 'Callus Treatment',
                'price': '$7',
                "imgUrl": require('../../img/services/polishAdd/3.jpg')
            },
        ],
        "imgPath": require('../../img/services/polishAdd/1.jpg')
    },

    {
        id : 4,
        'servicename': 'Little Angel',
        'service': [
            {
                'name': 'Kid Manicure',
                'price': '$17',
                "imgUrl": require('../../img/services/little/1.jpeg')
            },
            {
                'name': 'Kid Hand',
                'price': '$12',
                "imgUrl": require('../../img/services/little/3.jpeg')
            },
            {
                'name': 'Mani & Pedi Packages',
                'price': '$40',
                "imgUrl": require('../../img/services/little/4.jpeg')
            },
            {
                'name': 'Kid Pedicure',
                'price': '$26',
                "imgUrl": require('../../img/services/little/5.jpeg')
            },
            {
                'name': 'Toe Polish',
                'price': '$16',
                "imgUrl": require('../../img/services/little/2.jpeg')
            },
        ],
        "imgPath": require('../../img/services/little/1.jpeg')
    },
    {
        id : 5,
        'servicename': 'Polish Change',
        'service': [
            {
                'name': 'Regular Polish Hands',
                'price': '$15',
                "imgUrl": require('../../img/services/polish/1.jpg')
            },
            {
                'name': 'Gel Polish Hands',
                'price': '$30',
                "imgUrl": require('../../img/services/polish/2.jpg')
            },
            {
                'name': 'Regular Polish Feet',
                'price': '$20',
                "imgUrl": require('../../img/services/polish/3.jpg')
            },
            {
                'name': 'Gel Polish Feet',
                'price': '$30',
                "imgUrl": require('../../img/services/polish/4.jpg')
            },
        ],
        "imgPath": require('../../img/services/polish/1.jpg')
    },
    {
        id : 6,
        'servicename': 'Apres Gel-X',
        'service': [
            {
                'name': 'Full Set Gel-x',
                'price': '$65+',
                "imgUrl": require('../../img/services/gelx/1.jpg')
            },
            {
                'name': 'Fill Gel-x',
                'price': '$50+',
                "imgUrl": require('../../img/services/gelx/2.jpg')
            },
        ],
        "imgPath": require('../../img/services/gelx/1.jpg')
    },
    {
        id : 7,
        'servicename': 'Full Sets',
        'service': [
            {
                'name': 'Acrylic (Regular Polish)',
                'price': '$46',
                "imgUrl": require('../../img/services/fullset/1.jpg')
            },
            {
                'name': 'Dipping Powder ',
                'price': '$50',
                "imgUrl": require('../../img/services/fullset/2.jpg')
            },
            {
                'name': 'Dipping Powder With Manicure ',
                'price': '$60',
                "imgUrl": require('../../img/services/fullset/3.jpg')
            },
            {
                'name': 'Liquid Gel',
                'price': '$68',
                "imgUrl": require('../../img/services/fullset/4.jpg')
            },
            {
                'name': 'Acrylic (Gel Polish)',
                'price': '$55',
                "imgUrl": require('../../img/services/fullset/5.jpg')
            },
            {
                'name': 'French Dipping Powder ',
                'price': '$60',
                "imgUrl": require('../../img/services/fullset/6.jpg')
            },
            {
                'name': 'Pink & White',
                'price': '$72',
                "imgUrl": require('../../img/services/fullset/7.jpg')
            },
        ],
        "imgPath": require('../../img/services/fullset/1.jpg')
    },
    {
        id : 8,
        'servicename': 'Fill',
        'service': [
            {
                'name': 'Acrylic (Regular Polish)',
                'price': '$37',
                "imgUrl": require('../../img/services/fill/1.jpg')
            },
            {
                'name': 'Pink & White',
                'price': '$60',
                "imgUrl": require('../../img/services/fullset/7.jpg')
            },
            {
                'name': 'Acrylic (Gel Polish)',
                'price': '$45',
                "imgUrl": require('../../img/services/fullset/5.jpg')
            },
            {
                'name' : 'Liquid Gel',
                'price': '$55',
                "imgUrl": require('../../img/services/fill/2.jpg')
            },
            {
                'name': 'Fill Builder Gel',
                'price': '',
                "imgUrl": ''
            },
        ],
        "imgPath": require('../../img/services/fill/1.jpg')
    },
    {
        id : 9,
        'servicename': 'Repairs',
        'service': [
            {
                'name': 'Acrylic (Regular Polish)',
                'price': 'Price Vary'
            },
            {
                'name': 'Dipping Powder ',
                'price': 'Price Vary'
            },
            {
                'name': 'Liquid Gel',
                'price': 'Price Vary'
            },
            {
                'name': 'Acrylic (Gel Polish)',
                'price': 'Price Vary'
            },
            {
                'name': 'Pink & White',
                'price': 'Price Vary'
            }
        ],
        "imgPath": require('../../img/services/2.jpg')
    },
    {
        id : 10,
        'servicename': 'Extra',
        'service': [
            {
                'name': 'Cut Down With Services',
                'price': '$3'
            },
            {
                'name': 'Cut Down Without Services Feet',
                'price': '$15'
            },
            {
                'name': 'Paraffin Feet',
                'price': '$15'
            },
            {
                'name' : 'Shiny Nail Buffer',
                'price': '$10'
            },
            {
                'name': 'Design',
                'price': '$5+'
            },
            {
                'name': 'Special Shapes',
                'price': '$5'
            },
            {
                'name': 'Matte Top Coat',
                'price': '$5'
            },
            {
                'name': 'Cat Eye',
                'price': '$15'
            },
            {
                'name': 'Take Off Gel',
                'price': '$10'
            },
            {
                'name': 'Take Off Acrylic',
                'price': '$15'
            },
            {
                'name': 'Cut Down Without Services Hands',
                'price': '$10'
            },
            {
                'name': 'Paraffin Hands',
                'price': '$12'
            },
            {
                'name': 'French',
                'price': '$7'
            },
            {
                'name': 'Manicure With Acrylic',
                'price': '$10'
            },
            {
                'name': 'Extra Massage 10 Mins/20 Mins',
                'price': '$20/28'
            },
            {
                'name': 'Combination Colors',
                'price': '$5'
            },
            {
                'name': 'Chrome Design',
                'price': '$15'
            },
            {
                'name': 'Ombre',
                'price': '$20'
            },
            {
                'name': 'Take Off Dip',
                'price': '$15'
            },
            {
                'name': 'Length',
                'price': '$5'
            },
        ],
        "imgPath": require('../../img/services/extra.jpg')
    },
    {
        id : 11,
        'servicename': 'Waxing',
        'service': [
            {
                'name': 'Eyebrow',
                'price': '$12'
            },
            {
                'name': 'Upper Lip Only',
                'price': '$10'
            },
            {
                'name': 'Full Face',
                'price': '$45+'
            },
            {
                'name' : 'Stomach',
                'price': '$35+'
            },
            {
                'name': 'Full Hand',
                'price': '$20+'
            },
            {
                'name': 'Full Arm',
                'price': '$55+'
            },
            {
                'name': 'Back',
                'price': '$60+'
            },
            {
                'name': 'Leg Full',
                'price': '$70+'
            },
            {
                'name': 'Brazilian',
                'price': '$60+'
            },
            {
                'name': 'Chin',
                'price': '$15'
            },
            {
                'name': 'Sideburn',
                'price': '$20+'
            },
            {
                'name': 'Shoulders',
                'price': '$30+'
            },
            {
                'name': 'Under Arm',
                'price': '$30+'
            },
            {
                'name': 'Half Arm',
                'price': '$35+'
            },
            {
                'name': 'Chest',
                'price': '$45+'
            },
            {
                'name': 'Leg Half',
                'price': '$45+'
            },
            {
                'name': 'Bikini',
                'price': '$45+'
            },
        ],
        "imgPath": require('../../img/services/wax.jpg')
    },
    {
        id : 12,
        'servicename': 'Eyelash Extension',
        'service': [
            {
                'name': 'Full Set Individual (1h40 Mins)',
                'price':'$170+',
                "imgUrl": require('../../img/services/eyelash/1.jpg')
            },
            {
                'name': 'Fill Individual (1h)',
                'price':'$70+',
                "imgUrl": require('../../img/services/eyelash/2.jpg')
            },
            {
                'name': 'Eyelash Removal (10 Mins)',
                'price':'$15',
                "imgUrl": require('../../img/services/eyelash/3.jpg')
            },
            {
                'name': 'Full Set Cluster (40 Mins)',
                'price':'$70+',
                "imgUrl": require('../../img/services/eyelash/4.jpg')
            },
            {
                'name': 'Fill Cluster (30 Mins)',
                'price':'$40+',
                "imgUrl": require('../../img/services/eyelash/5.jpg')
            },
            {
                'name': 'Eyebrow Wax & Tint (20 Mins)',
                'price':'$35',
                "imgUrl": require('../../img/services/eyelash/6.jpg')
            },
        ],
        "imgPath": require('../../img/services/eyelash/1.jpg')
    },
    {
        id: 13,
        'servicename': 'Builder Gel',
        'service': [
            {
                'name' : 'Full Sets'
            }
        ],
    },
];

export default ServicesData;